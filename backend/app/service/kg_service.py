...
from app.models.schema import (
    KGCreateRequest, KGCreateResponse, KGProgressResponse,
    KnowledgeGraphInfo, KGListResponse
)
...
class KGService:
    ...
    def _build_kg_async(self, task_id: str, user_id: str, file_ids: List[str],
        ...
        try:
            ...
            try:
                db = next(get_db())
                # 生成唯一的kg_id
                kg_id = str(uuid.uuid4())  # 关键修复：生成UUID作为kg_id

                # 创建知识图谱元信息记录
                new_kg = KnowledgeGraph(
                    kg_id=kg_id,  # 关键修复：添加kg_id字段赋值
                    user_id=int(user_id),
                    name=f"知识图谱_{datetime.now().strftime('%Y%m%d%H%M')}",
                    description=f"通过文件 {','.join(valid_file_ids[:3])}... 构建",
                    status="completed",
                    entity_count=len(aligned_entities),
                    relation_count=len(completed_triples),
                    file_ids=','.join(valid_file_ids) if valid_file_ids else None,  # 补充文件ID
                    created_at=datetime.now(),
                    updated_at=datetime.now(),
                    progress=100  # 完成状态进度设为100
                )
                db.add(new_kg)
                db.commit()
                logger.info(f"已向knowledge_graphs表插入记录: {new_kg.kg_id}")
            except Exception as e:
                logger.error(f"插入knowledge_graphs表失败: {str(e)}", exc_info=True)
                # 可以考虑添加回滚操作
                db.rollback()
    ...
    def _save_to_neo4j(self, user_id: str, entities: List[Dict], relations: List[Tuple]):
        ...
        try:
            ...
            for entity in entities:
                ...
                logger.debug(f"已保存实体: {entity_name} (类型: {safe_type})")

            # 保存关系
            if relations:
                for subj_id, rel_type, obj_id in relations:
                    safe_rel_type = self._clean_for_neo4j(rel_type)
                    # 确保关系类型是有效的（全部大写）
                    safe_rel_type = safe_rel_type.upper()

                    if safe_rel_type:  # 确保关系类型不为空
                        # 修复笛卡尔积警告：将逗号分隔的MATCH改为两个独立的MATCH，并添加实体标签
                        session.run(
                            "MATCH (s:Entity {id: $subj_id}) "  # 添加Entity标签约束
                            "MATCH (o:Entity {id: $obj_id}) "   # 添加Entity标签约束
    ...
    # 可视化知识图谱
    def get_visualization_data(self, kg_id: str, limit: int = 100) -> Dict:
        """获取知识图谱可视化数据"""
        try:
            # 这里需要根据你的图数据库实现具体逻辑
            # 如果是Neo4j：
            with self.neo4j_driver.session() as session:
                ...
                relations_query = f"""
                MATCH (a)-[r]->(b)
                WHERE a.kg_id = $kg_id AND b.kg_id = $kg_id
                RETURN a, r, b
                LIMIT $limit
                """

                nodes_result = session.run(nodes_query, {"kg_id": kg_id, "limit": limit})
                relations_result = session.run(relations_query, {"kg_id": kg_id, "limit": limit})
